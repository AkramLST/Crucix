...
export async function briefing(apiKey) {
  ...
  if (wti && wti.value > 100)
    signals.push(`WTI crude above $100 at $${wti.value}/bbl`);
  if (wti && wti.value < 50)
    signals.push(
      `WTI crude below $50 at $${wti.value}/bbl — supply glut or demand destruction`,
    );
  if (brent && wti && brent.value - wti.value > 10) {
    signals.push(
      `Brent-WTI spread wide at $${(brent.value - wti.value).toFixed(2)} — supply/logistics divergence`,
    );
  }
  ...
  if (gas && gas.value > 6)
    signals.push(`Natural gas elevated at $${gas.value}/MMBtu`);
  if (gas && gas.value > 9)
    signals.push(`Natural gas crisis-level at $${gas.value}/MMBtu`);
  ...
  if (invRecent.length >= 2) {
    ...
    if (Math.abs(weekChange) > 5000) {
      ...
      signals.push(
        `Large crude inventory ${direction}: ${weekChange > 0 ? "+" : ""}${(weekChange / 1000).toFixed(1)}M barrels`,
      );
    }
  }
  ...
  return {
    ...
    oilPrices: {
      wti: wti
        ? { ...wti, label: OIL_SERIES.wti.label, recent: wtiRecent }
        : null,
      brent: brent
        ? { ...brent, label: OIL_SERIES.brent.label, recent: brentRecent }
        : null,
    ...
    inventories: {
      crudeStocks: inv
        ? {
            ...inv,
            label: INVENTORY_SERIES.crudeStocks.label,
            recent: invRecent,
          }
        : null,
    },
...
if (process.argv[1]?.endsWith("eia.mjs")) {
  const data = await briefing("Ryh0tHhej4ehIuKs1S5ser9lUSuqoSNs9y3I763E");
