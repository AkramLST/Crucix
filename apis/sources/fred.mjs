...
export async function briefing(apiKey) {
  if (!apiKey) {
    return {
      source: "FRED",
      error:
        "No FRED API key. Get one free at https://fred.stlouisfed.org/docs/api/api_key.html",
  ...
  const signals = [];
  if (yieldCurve10y2y !== null && yieldCurve10y2y < 0)
    signals.push("YIELD CURVE INVERTED (10Y-2Y) — recession signal");
  if (yieldCurve10y3m !== null && yieldCurve10y3m < 0)
    signals.push("YIELD CURVE INVERTED (10Y-3M) — stronger recession signal");
  if (vix !== null && vix > 30)
    signals.push(`VIX ELEVATED at ${vix} — high fear/volatility`);
  if (vix !== null && vix > 40)
    signals.push(`VIX EXTREME at ${vix} — crisis-level fear`);
  if (hySpread !== null && hySpread > 5)
    signals.push(`HIGH YIELD SPREAD WIDE at ${hySpread}% — credit stress`);
...
if (process.argv[1]?.endsWith("fred.mjs")) {
  const data = await briefing("fcbaa9366d1ee7d10a4d43b94fa04193");
